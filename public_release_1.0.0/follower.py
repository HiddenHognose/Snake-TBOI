...
class Follower:
    ...
    def update(self, enemies, bosses, bullets):
        ...
        # OPTIMIZATION: Find nearest enemy or boss using squared distance (avoid sqrt)
        nearest_target = None
        nearest_dist_sq = float('inf')
        range_sq = 500 * 500  # 500 range squared
        ...
        # Shoot at nearest target (only calculate sqrt when actually shooting)
        if nearest_target and nearest_dist_sq < range_sq:
            if self.shoot_cooldown <= 0:
                dx = nearest_target.x - self.x
                dy = nearest_target.y - self.y
                dist_sq = dx*dx + dy*dy
                if dist_sq > 0:
                    dist = math.sqrt(dist_sq)  # Only calculate sqrt once when needed
                    dx /= dist
                    dy /= dist
