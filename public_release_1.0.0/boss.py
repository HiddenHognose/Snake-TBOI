...
import math
import random
from enemy_types import get_lizard_sprite_base, recolor_sprite_with_palette, PALETTE_ROW_MAPPING

class Boss:
    def __init__(self, x, y, testing_mode=False):
        self.x = x
        self.y = y
        # Make boss smaller in test mode for easier viewing
        if testing_mode:
            self.radius = 30  # Smaller size for test mode
        else:
            self.radius = 80  # BIIIIIG LEEZARD
        self.speed = 1.2
        self.max_health = 15
        self.health = self.max_health
        # Neon red for boss (kept for fallback drawing)
    ...
    def _load_sprite(self):
        """Load and recolor the boss sprite using palette row 3 - BIIIIIG LEEZARD"""
        base_sprite = get_lizard_sprite_base()
        if base_sprite:
            # Boss uses row 3 (index 2)
            palette_row = PALETTE_ROW_MAPPING.get('boss', 2)
    ...
    def update(self, player_x, player_y, bullets):
        ...
        # OPTIMIZATION: Normal movement towards player (use squared distance, only sqrt when needed)
        dx = player_x - self.x
        dy = player_y - self.y
        dist_sq = dx*dx + dy*dy
        
        if dist_sq > 0:
            dist = math.sqrt(dist_sq)  # Only calculate sqrt once
            # Normalize direction
            dx /= dist
            dy /= dist
        ...
        if self.attack_cooldown > 0:
            ...
            # Start charge attack (dist already calculated above)
            if dist_sq > 0 and random.random() < 0.3:  # 30% chance to charge
                self.charging = True
