...
class Item:
    ...
    def collides_with(self, other):
        """Check collision with player - OPTIMIZED with squared distance"""
        dx = self.x - other.x
        dy = self.y - other.y
        dist_sq = dx*dx + dy*dy
    ...
    def draw(self, screen):
        ...
        if self.item_type == 'health':
            ...
            # Default drawing for any item type not specifically handled
            # Draw a generic item (colored circle with border)
            pygame.draw.circle(screen, self.color, (int(self.x), int(self.y)), self.radius)
            pygame.draw.circle(screen, (255, 255, 255), (int(self.x), int(self.y)), self.radius - 3)
            # Draw a star pattern for generic items
            for i in range(5):
                angle = (2 * math.pi / 5) * i - math.pi / 2
                x1 = int(self.x + math.cos(angle) * 6)
                y1 = int(self.y + math.sin(angle) * 6)
